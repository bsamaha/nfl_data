-- queries/utilization/materialize_player_week.sql
-- Materialize current-season player-week utilization as partitioned Parquet
WITH params AS (
...
COPY (
  SELECT * FROM (
    -- Inline the main SELECT from player_week_utilization.sql
    WITH w AS (
...
) TO 'data/gold/utilization/player_week/season=' || CAST((SELECT season FROM params) AS VARCHAR) || '/week=' || CAST(week AS VARCHAR) || '/part.parquet' (FORMAT PARQUET);
